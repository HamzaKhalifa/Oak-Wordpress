    ...
    for (var i = 0; i < updateButtons.length; i++) {
        updateButtons[i].addEventListener('click', function() {
            ...
            for (var j = 0; j < DATA.objects.length; j++) {
                if (DATA.objects[j].object_identifier == this.getAttribute('object-identifier')) {
                    whichOranization = DATA.objects[j];
                }
            }
            window.location.replace(DATA.adminUrl + 'admin.php?page=oak_model_add_' + DATA.modelIdentifier + '&object_identifier=' + whichOranization.object_identifier);
        });
    }
}
