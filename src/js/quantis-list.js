    ...
    for (var i = 0; i < updateButtons.length; i++) {
        updateButtons[i].addEventListener('click', function() {
            ...
            for (var j = 0; j < DATA.quantis.length; j++) {
                if (DATA.quantis[j].quanti_identifier == this.getAttribute('quanti-identifier')) {
                    whichOranization = DATA.quantis[j];
                }
            }
            window.location.replace(DATA.adminUrl + 'admin.php?page=oak_add_quanti&quanti_identifier=' + whichOranization.quanti_identifier);
        });
    }
}
