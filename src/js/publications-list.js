    ...
    for (var i = 0; i < updateButtons.length; i++) {
        updateButtons[i].addEventListener('click', function() {
            ...
            for (var j = 0; j < DATA.publications.length; j++) {
                if (DATA.publications[j].publication_identifier == this.getAttribute('publication-identifier')) {
                    whichOranization = DATA.publications[j];
                }
            }
            window.location.replace(DATA.adminUrl + 'admin.php?page=oak_add_publication&publication_identifier=' + whichOranization.publication_identifier);
        });
    }
}
