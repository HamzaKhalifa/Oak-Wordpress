    ...
    for (var i = 0; i < updateButtons.length; i++) {
        updateButtons[i].addEventListener('click', function() {
            ...
            for (var j = 0; j < DATA.terms.length; j++) {
                if (DATA.terms[j].term_identifier == this.getAttribute('term-identifier')) {
                    whichOranization = DATA.terms[j];
                }
            }
            window.location.replace(DATA.adminUrl + 'admin.php?page=oak_taxonomy_add_' + DATA.taxonomyIdentifier + '&term_identifier=' + whichOranization.term_identifier);
        });
    }
}
