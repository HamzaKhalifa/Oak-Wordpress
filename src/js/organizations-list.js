    ...
    for (var i = 0; i < updateButtons.length; i++) {
        updateButtons[i].addEventListener('click', function() {
            ...
            for (var j = 0; j < DATA.organizations.length; j++) {
                if (DATA.organizations[j].organization_identifier == this.getAttribute('organization-identifier')) {
                    whichOranization = DATA.organizations[j];
                }
            }
            window.location.replace(DATA.adminUrl + 'admin.php?page=oak_add_organization&organization_identifier=' + whichOranization.organization_identifier);
        });
    }
}
