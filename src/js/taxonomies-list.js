    ...
    for (var i = 0; i < updateButtons.length; i++) {
        updateButtons[i].addEventListener('click', function() {
            ...
            for (var j = 0; j < DATA.taxonomies.length; j++) {
                if (DATA.taxonomies[j].taxonomy_identifier == this.getAttribute('taxonomy-identifier')) {
                    whichOranization = DATA.taxonomies[j];
                }
            }
            window.location.replace(DATA.adminUrl + 'admin.php?page=oak_add_taxonomy&taxonomy_identifier=' + whichOranization.taxonomy_identifier);
        });
    }
}
