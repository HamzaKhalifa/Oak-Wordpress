    ...
    for (var i = 0; i < updateButtons.length; i++) {
        updateButtons[i].addEventListener('click', function() {
            ...
            for (var j = 0; j < DATA.qualis.length; j++) {
                if (DATA.qualis[j].quali_identifier == this.getAttribute('quali-identifier')) {
                    whichOranization = DATA.qualis[j];
                }
            }
            window.location.replace(DATA.adminUrl + 'admin.php?page=oak_add_quali&quali_identifier=' + whichOranization.quali_identifier);
        });
    }
}
